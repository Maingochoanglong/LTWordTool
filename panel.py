...
class FilePickerRow(QWidget):
    ...
    def __init__(self, label_text, save_mode=False, default_name="",
        ...
        self.button = QPushButton("Chọn Nơi Lưu" if save_mode else "Chọn File")
        self.button.clicked.connect(self.pick_file)

        self.field = QWidget()
        field_layout = QHBoxLayout(self.field)
        field_layout.setContentsMargins(0, 0, 0, 0)
        field_layout.addWidget(self.edit, 1)
        ...
        if form_layout is None:
            ...
            if not isinstance(form_layout, QFormLayout):
                raise TypeError("form_layout must be a QFormLayout")
            form_layout.addRow(self.label, self.field)
...
class RunPanel(QWidget):
    ...
    def __init__(self, run_label, file_rows, result_row, log_height=150):
        ...
        self.clear_cache_button = QPushButton("Xóa Cache")
    ...
    def _on_clear_cache_clicked(self):
        ...
        try:
            path = clear_app_cache()
        except OSError as exc:
            QMessageBox.critical(self, "Không Thể Xóa Cache", friendly_error_text(exc))
            return
        QMessageBox.information(
            self,
            "Đã Xóa Cache",
            f"Đã xóa file cấu hình .ini trên máy này:\n{path}\n\n"
